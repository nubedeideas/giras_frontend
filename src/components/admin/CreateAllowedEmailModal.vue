...
<template>
  <AppModal :show="show" @close="emit('close')">
    <p class="text-base font-bold text-ink tracking-[-0.2px] mb-4">Invitar cliente</p>

    <div class="space-y-3">
      <div>
        <label :class="labelClass">Email *</label>
        <input v-model="email" type="email" :class="inputClass" placeholder="cliente@ejemplo.com" />
      </div>
      <div>
        <label :class="labelClass">Notas</label>
        <textarea v-model="notes" :class="inputClass" rows="2" placeholder="Contexto interno (opcional)" style="resize: vertical" />
      </div>
      ...
      <div class="flex justify-end gap-2 pt-1">
        ...
        <button
          ...
          @click="submit"
        >{{ saving ? 'Invitando…' : 'Invitar' }}</button>
      </div>
    </div>
  </AppModal>
</template>
