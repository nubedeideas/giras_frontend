...
<template>
  <div class="flex flex-col h-full">
    <div class="flex-1 overflow-y-auto px-4 pt-4">
      <SubscriptionStatsPanel v-if="stats" :stats="stats" />

      <div class="mb-3">
        <p class="text-base font-bold text-ink tracking-[-0.2px] mb-3">Suscripciones</p>
        <div class="flex gap-2">
          <div class="flex-1 flex items-center gap-[7px] bg-glass border border-line rounded-sm px-[11px] py-[7px]">
            ...
            <input v-model="search" placeholder="Buscar por gira o cliente…" class="flex-1 bg-transparent border-none outline-none text-ink text-[12px] placeholder:text-ink-4" />
          </div>
