  ...
  <div class="bg-glass border border-line rounded-xl overflow-hidden">
    ...
    >
      <div class="flex-1 min-w-0">
        <p class="text-[12px] font-medium text-ink truncate">{{ payment.user_email }}</p>
        <p class="text-[10px] text-ink-4 truncate">{{ payment.tour_name }} · {{ formatDate(payment.payment_date ?? payment.created_at) }}</p>
      </div>
      <p class="text-[12px] font-semibold text-ink flex-shrink-0">{{ payment.amount }} {{ payment.currency }}</p>
      <span
    ...
    <div v-if="expanded" class="border-t border-line px-3 py-2.5 space-y-1.5">
      ...
      <template v-else-if="detail">
        <p class="text-[11px] text-ink-3">{{ detail.user_full_name }}</p>
        <p v-if="detail.subscription_plan" class="text-[11px] text-ink-3">Plan: {{ detail.subscription_plan }}</p>
        <p v-if="detail.authorization_code" class="text-[11px] text-ink-4">Código: {{ detail.authorization_code }}</p>
        <p v-if="detail.notes" class="text-[11px] text-ink-4">{{ detail.notes }}</p>
