...
import CustomPoolCard from '@/components/admin/CustomPoolCard.vue'
import EditCustomPoolModal from '@/components/admin/EditCustomPoolModal.vue'
import AdminPagination from '@/components/admin/AdminPagination.vue'

const api = useAdminCustomPools()
const items = ref<CustomPool[]>([])
const count = ref(0)
const next = ref<string | null>(null)
const previous = ref<string | null>(null)
const loading = ref(false)
const search = ref('')
const editing = ref<CustomPool | null>(null)
  ...
  <div class="flex flex-col h-full">
    <div class="px-4 pt-4 pb-3 border-b border-line flex-shrink-0">
      <div class="flex items-center justify-between mb-3">
        <p class="text-base font-bold text-ink tracking-[-0.2px]">Cupos Custom</p>
        <button
      ...
      <div class="flex items-center gap-[7px] bg-glass border border-line rounded-sm px-[11px] py-[7px]">
        ...
        <input v-model="search" placeholder="Buscar por email de cliente…" class="flex-1 bg-transparent border-none outline-none text-ink text-[12px] placeholder:text-ink-4" />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-2 space-y-1.5">
      ...
      <CustomPoolCard v-for="pool in items" :key="pool.uuid" :pool="pool" @edit="editing = pool" />
    </div>

    <AdminPagination :count="count" :next="next" :previous="previous" @next="load(next!)" @prev="load(previous!)" />

    <EditCustomPoolModal :show="showCreate || !!editing" :pool="editing" @close="showCreate = false; editing = null" @saved="load()" />
  </div>
</template>
