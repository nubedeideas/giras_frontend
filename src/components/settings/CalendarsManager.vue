...
<template>
  ...
  <div class="bg-[rgba(168,216,0,0.06)] border border-[rgba(168,216,0,0.15)] rounded-xl px-3.5 py-3 mb-4 space-y-1">
    <p class="text-[11px] font-semibold text-acid">
      ¿Cómo funciona?
    </p>
  ...
  <div
    v-if="showForm"
    class="bg-glass border border-line rounded-xl px-3.5 py-3 mb-3 space-y-2.5"
  >
    ...
    <div class="space-y-1.5">
      ...
      <!-- Loading state -->
      <div
        v-if="loadingGoogleCals"
        class="flex items-center gap-2 py-2"
      >
        <svg
          class="animate-spin text-ink-4 flex-shrink-0"
          width="13"
          height="13"
          viewBox="0 0 24 24"
          fill="none"
      ...
      <template v-else>
        <div class="space-y-1">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-[9px] text-ink-4 font-mono truncate">
                {{ cal.id }}
              </p>
            </div>
            ...
            >
              <div
                v-if="formCalId === cal.id"
                class="w-1 h-1 rounded-full bg-black"
              />
            </div>
          </button>
        </div>
        ...
        <p
          v-if="googleCals.length === 0"
          class="text-[10px] text-ink-4 py-1"
        >
  ...
  <p
    v-else-if="error"
    class="text-[11px] text-red-400 py-2"
  >
    {{ error }}
  </p>
  ...
  <!-- Empty -->
  <div
    v-else-if="calendars.length === 0 && !showForm"
    class="text-center py-6 space-y-1.5"
  >
    <svg
      width="26"
      height="26"
    ...
    <p class="text-[10px] text-ink-4">
      Pulsa "Conectar" para añadir tu Google Calendar
    </p>
  </div>
    ...
    >
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[10px] text-ink-4 font-mono mt-0.5 truncate">
            {{ cal.calendar_id }}
          </p>
          <p class="text-[10px] text-ink-4 mt-0.5">
            Últ. sync: {{ formatDate(cal.last_synced_at) }}
          </p>
        </div>
      </div>
      ...
      <!-- Assigned tours -->
      <div
        v-if="toursByCalendar.get(cal.id)?.length"
